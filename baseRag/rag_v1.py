...
def calc_embedings(paragraphs):
    return [
        client.embeddings(model="mistral", prompt=data)["embedding"]
...
def cache_embeddings(filename,paragraphs):
    ...
    embeddings =cal_embeddings(paragraphs)

    with open(embedding_file,"w") as f:
        jsom.dump(embeddings,f)
...
if __name__== "__main__":
    ...
    while prompt.lower() != "bye":
        prompt_embedding = client.embeddings(model="mistral", prompt=prompt)["embedding"]
